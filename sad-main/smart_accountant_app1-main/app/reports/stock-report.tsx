...
export default function StockReportScreen() {
  ...
  return (
    <View style={[styles.c, { paddingTop: insets.top }]}>
      <View style={styles.h}><TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>←</Text></TouchableOpacity><Text style={styles.t}>📦 تقرير المخزون</Text><TouchableOpacity onPress={loadData}><Text style={styles.refresh}>🔄</Text></TouchableOpacity></View>
      <FlatList data={products} keyExtractor={i=>i.id.toString()}
        renderItem={({item}) => (<View style={styles.row}><Text style={styles.name}>{item.name_ar}</Text><Text style={styles.price}>{(item.purchase_price||0).toLocaleString()} ﷼</Text></View>)}
...
const styles = StyleSheet.create({
  ...
  name:{color:'#FFF',fontSize:13},price:{color:'#10B981',fontSize:13,fontWeight:'bold'},
