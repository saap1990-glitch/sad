...
export default function CashReceiptScreen() {
  ...
  const sourceTypes = [
    { key: 'cash' as const, label: '💰 صندوق', parentCode: '1101', prefix: 'RCP-CSH-' },
    { key: 'bank' as const, label: '🏦 بنك', parentCode: '1102', prefix: 'RCP-BNK-' },
    { key: 'exchange' as const, label: '💱 صرافة', parentCode: '1103', prefix: 'RCP-EXC-' },
    { key: 'wallet' as const, label: '📱 محفظة', parentCode: '1104', prefix: 'RCP-WLT-' },
  ];
  ...
  const saveReceipt = async () => {
    if (!amount || !selectedSource || !selectedCredit || !db) { Alert.alert('خطأ', 'أكمل البيانات'); return; }
    ...
    try {
      ...
      await financialEngine.executeTransaction(db, {
        date,
        description: description || `سند قبض ${receiptNumber}`,
  ...
  return (
    <View style={[styles.c,{paddingTop:insets.top}]}>
      <View style={styles.h}><TouchableOpacity onPress={()=>router.back()}><Text style={styles.back}>←</Text></TouchableOpacity><Text style={styles.t}>📥 سند قبض</Text><Text style={styles.num}>{receiptNumber}</Text></View>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Text style={styles.section}>الحساب الدائن</Text>
        <TouchableOpacity style={styles.picker} onPress={()=>setShowCreditPicker(true)}><Text style={selectedCredit?styles.pv:styles.pp}>{selectedCredit?`${selectedCredit.code} - ${selectedCredit.name_ar}`:'اختر...'}</Text><Text>▼</Text></TouchableOpacity>
        ...
        {selectedSource&&selectedCredit&&amount&&(
          <View style={styles.preview}><Text style={styles.previewT}>📋 معاينة القيد</Text>
            ...
            <Text style={styles.previewL}>💰 {selectedCredit.code} دائن: {previewBaseAmount.toLocaleString()} ﷼</Text>
          </View>
        )}
      ...
      <PickerModal visible={showCreditPicker} title="اختر الحساب الدائن" data={creditAccounts} onSelect={(item)=>{setSelectedCredit(item);setShowCreditPicker(false)}} onClose={()=>setShowCreditPicker(false)} />
...
const styles = StyleSheet.create({
  ...
  back:{fontSize:22,color:'#D4AF37'},t:{color:'#D4AF37',fontSize:17,fontWeight:'bold'},num:{color:'#10B981',fontSize:11},
  content:{padding:14},
  ...
  bh:{color:'#10B981',fontSize:11,marginTop:4,textAlign:'right'},
  ...
  auto:{backgroundColor:'#16213E',borderRadius:10,padding:14,borderWidth:1,borderColor:'#10B98140'},
  autoT:{color:'#10B981',fontSize:13},
  ...
  saveBtn:{flex:2,padding:14,borderRadius:12,backgroundColor:'#10B981',alignItems:'center'},
