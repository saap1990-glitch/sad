...
export default function CashBoxesScreen() {
  ...
  const [walletsBoxes, setCashBoxes] = useState<any[]>([]);
  ...
  const saveCashBox = async () => {
    if (!form.name_ar.trim() || !db) { Alert.alert('خطأ', 'أدخل اسم المحفظة'); return; }
    setLoading(true);
    try {
      if (editingId) {
        await db.runAsync('UPDATE wallets SET name_ar=? WHERE id=?', [form.name_ar, editingId]);
        Alert.alert('✅', 'تم تحديث المحفظة');
      } else {
        // 1. إنشاء المحفظة (الكيان)
        const result = await db.runAsync('INSERT INTO wallets (name_ar, is_active) VALUES (?,1)', [form.name_ar]);
        ...
        await accountCreationService.createChildAccount(db, {
          parentSystemKey: 'wallets_account',
          nameAr: form.name_ar,
        ...
        Alert.alert('✅', `تم إضافة المحفظة: ${form.name_ar}`);
      }
  ...
  return (
    <View style={[styles.c, { paddingTop: insets.top }]}>
      <View style={styles.h}><TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>←</Text></TouchableOpacity><Text style={styles.t}>📱 المحافظ</Text><TouchableOpacity onPress={() => { setForm({ name_ar: '', opening_balance: '0', currency_code: 'YER' }); setEditingId(null); setShowForm(true); }} style={styles.add}><Text style={styles.addT}>+</Text></TouchableOpacity></View>
      <FlatList data={walletsBoxes} keyExtractor={i => i.id.toString()}
        renderItem={({ item }) => (
      ...
      <Modal visible={showForm} animationType="slide" transparent>
        <View style={styles.modal}><View style={styles.modalC}><Text style={styles.modalT}>{editingId ? '✏️ تعديل' : '➕ إضافة محفظة'}</Text>
          <TextInput style={styles.inp} value={form.name_ar} onChangeText={v => setForm({...form, name_ar: v})} placeholder="اسم المحفظة *" placeholderTextColor="#666" textAlign="right" />
...
const styles = StyleSheet.create({
  ...
  add:{backgroundColor:'#6366F1',width:36,height:36,borderRadius:18,justifyContent:'center',alignItems:'center'},addT:{color:'#FFF',fontSize:22},
  list:{padding:16},
  ...
  icon:{fontSize:28,marginRight:12},name:{fontSize:16,fontWeight:'600'},accountCode:{color:'#D4AF37',fontSize:11},balance:{fontSize:14,fontWeight:'bold',color:'#6366F1'},baseBalance:{fontSize:9,color:'#D4AF37'},
  ...
  save:{flex:2,padding:14,borderRadius:8,backgroundColor:'#6366F1',alignItems:'center'},saveT:{color:'#FFF',fontWeight:'bold'},
});
