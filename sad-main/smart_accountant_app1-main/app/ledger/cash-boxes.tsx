...
export default function CashBoxesScreen() {
  ...
  const saveCashBox = async () => {
    if (!form.name_ar.trim() || !db) { Alert.alert('خطأ', 'أدخل اسم الصندوق'); return; }
    setLoading(true);
    try {
      if (editingId) {
        await db.runAsync('UPDATE cash_boxes SET name_ar=? WHERE id=?', [form.name_ar, editingId]);
        Alert.alert('✅', 'تم تحديث الصندوق');
      } else {
        // 1. إنشاء الصندوق (الكيان)
        const result = await db.runAsync('INSERT INTO cash_boxes (name_ar, is_active) VALUES (?,1)', [form.name_ar]);
        ...
        Alert.alert('✅', `تم إضافة الصندوق: ${form.name_ar}`);
      }
  ...
  return (
    <View style={[styles.c, { paddingTop: insets.top }]}>
      <View style={styles.h}><TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>←</Text></TouchableOpacity><Text style={styles.t}>💰 الصناديق</Text><TouchableOpacity onPress={() => { setForm({ name_ar: '', opening_balance: '0', currency_code: 'YER' }); setEditingId(null); setShowForm(true); }} style={styles.add}><Text style={styles.addT}>+</Text></TouchableOpacity></View>
      <FlatList data={cashBoxes} keyExtractor={i => i.id.toString()}
        renderItem={({ item }) => (
      ...
      <Modal visible={showForm} animationType="slide" transparent>
        <View style={styles.modal}><View style={styles.modalC}><Text style={styles.modalT}>{editingId ? '✏️ تعديل' : '➕ إضافة صندوق'}</Text>
          <TextInput style={styles.inp} value={form.name_ar} onChangeText={v => setForm({...form, name_ar: v})} placeholder="اسم الصندوق *" placeholderTextColor="#666" textAlign="right" />
...
const styles = StyleSheet.create({
  ...
  icon:{fontSize:28,marginRight:12},name:{fontSize:16,fontWeight:'600'},accountCode:{color:'#D4AF37',fontSize:11},balance:{fontSize:14,fontWeight:'bold',color:'#10B981'},baseBalance:{fontSize:9,color:'#D4AF37'},
