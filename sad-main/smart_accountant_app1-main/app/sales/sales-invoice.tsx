...
export default function SalesInvoiceScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    const last = await db.getFirstAsync("SELECT invoice_number FROM sales_invoices ORDER BY id DESC LIMIT 1") as any;
    let n = 1; if (last?.invoice_number) n = parseInt(last.invoice_number.split('-').pop()) + 1;
    setInvoiceNumber(`INV-S-${String(n).padStart(5,'0')}`);
    setLoading(false);
  }, [db]);
  ...
  const saveInvoice = async () => {
    ...
    if (paymentType === 'credit' && !selectedCustomer) { Alert.alert('خطأ', 'اختر العميل'); return; }
    if (paymentType === 'cash' && !selectedCashAccount && !selectedBankAccount) { Alert.alert('خطأ', 'اختر الصندوق أو البنك'); return; }

    setLoading(true);
    try {
      await db.runAsync('INSERT INTO sales_invoices (invoice_number, date, customer_id, total, net_total, status) VALUES (?,?,?,?,?,"posted")',
      ...
      if (!salesAccount) { Alert.alert('خطأ', 'حساب المبيعات غير موجود'); setLoading(false); return; }

      const lines: any[] = [];
      if (paymentType === 'cash') {
        if (selectedCashAccount) lines.push({ account_id: selectedCashAccount.id, debit_original: totalAmount, credit_original: 0, description: 'مبيعات نقدية' });
        if (selectedBankAccount) lines.push({ account_id: selectedBankAccount.id, debit_original: totalAmount, credit_original: 0, description: 'مبيعات بنكية' });
      } else {
        const custLink = await db.getFirstAsync("SELECT account_id FROM account_links WHERE module='customers' AND entity_id=?", [selectedCustomer.id]) as any;
      ...
      lines.push({ account_id: salesAccount.id, debit_original: 0, credit_original: totalAmount, description: 'إيراد مبيعات' });

      await financialEngine.executeTransaction(db, {
        date: invoiceDate,
        description: description || `فاتورة مبيعات ${invoiceNumber}`,
  ...
  return (
    <View style={[styles.c, { paddingTop: insets.top }]}>
      <View style={styles.h}><TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>←</Text></TouchableOpacity><Text style={styles.t}>🧾 فاتورة مبيعات</Text><Text style={styles.num}>{invoiceNumber}</Text></View>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {/* اختيار العميل أو الصندوق/البنك */}
        {paymentType === 'credit' ? (
          <>
