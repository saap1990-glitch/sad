...
export default function CustomerSalesScreen() {
  ...
  const [products, setCustomers] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  const loadData = useCallback(async () => {
    if (!db) return;
    const result = await db.getAllAsync(`
      SELECT p.name_ar, COUNT(si.id) as invoice_count, COALESCE(SUM(si.total),0) as total_sales
      FROM products c LEFT JOIN sales_invoices si ON si.product_id = c.id
  ...
  return (
    <View style={[styles.c, { paddingTop: insets.top }]}>
      <View style={styles.h}><TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>←</Text></TouchableOpacity><Text style={styles.t}>📊 مبيعات الأصناف</Text><View style={{width:40}}/></View>
