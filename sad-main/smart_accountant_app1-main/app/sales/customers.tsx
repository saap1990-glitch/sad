...
export default function CustomersScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    const result = await db.getAllAsync(`
      ...
      FROM customers c
      LEFT JOIN account_links al ON al.entity_id = c.id AND al.module = 'customers'
  ...
  const saveCustomer = async () => {
    if (!form.name_ar.trim() || !db) { Alert.alert('خطأ', 'أدخل اسم العميل'); return; }
    setLoading(true);
    try {
      if (editingId) {
        await db.runAsync('UPDATE customers SET name_ar=?, phone=?, email=?, address=? WHERE id=?', [form.name_ar, form.phone, form.email, form.address, editingId]);
        Alert.alert('✅', 'تم تحديث العميل');
      } else {
        const result = await db.runAsync('INSERT INTO customers (name_ar, phone, email, address, is_active) VALUES (?,?,?,?,1)', [form.name_ar, form.phone, form.email, form.address]);
        await accountCreationService.createChildAccount(db, {
          parentSystemKey: 'customer_parent',
          nameAr: form.name_ar,
        ...
        Alert.alert('✅', 'تم إضافة العميل وربطه محاسبياً');
      }
  ...
  return (
    <View style={[styles.c, { paddingTop: insets.top }]}>
      <View style={styles.h}><TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>←</Text></TouchableOpacity><Text style={styles.t}>👤 العملاء</Text><TouchableOpacity onPress={() => { setForm({ name_ar: '', phone: '', email: '', address: '', opening_balance: '0', currency_code: 'YER' }); setEditingId(null); setShowForm(true); }} style={styles.add}><Text style={styles.addT}>+</Text></TouchableOpacity></View>
      ...
      <Modal visible={showForm} animationType="slide" transparent>
        <View style={styles.modal}><View style={styles.modalC}><Text style={styles.modalT}>{editingId ? '✏️ تعديل' : '➕ إضافة عميل'}</Text>
          <TextInput style={styles.inp} value={form.name_ar} onChangeText={v => setForm({...form, name_ar: v})} placeholder="اسم العميل *" placeholderTextColor="#666" textAlign="right" />
