...
export default function CategoriesScreen() {
  ...
  return (
    <View style={[styles.c, { paddingTop: insets.top }]}>
      <View style={styles.h}><TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>←</Text></TouchableOpacity><Text style={styles.t}>📁 التصنيفات</Text><TouchableOpacity onPress={() => { setForm({ name_ar: '' }); setEditingId(null); setShowForm(true); }} style={styles.add}><Text style={styles.addT}>+</Text></TouchableOpacity></View>
      <FlatList data={categories} keyExtractor={i => i.id.toString()}
        ...
        ListEmptyComponent={<Text style={styles.empty}>لا توجد تصنيفات</Text>}
        contentContainerStyle={styles.list}
      />
      <Modal visible={showForm} animationType="slide" transparent>
        <View style={styles.modal}><View style={styles.modalC}><Text style={styles.modalT}>{editingId ? '✏️ تعديل' : '➕ إضافة تصنيف'}</Text>
          <TextInput style={styles.inp} value={form.name_ar} onChangeText={v => setForm({...form, name_ar: v})} placeholder="اسم التصنيف" placeholderTextColor="#666" textAlign="right" />
