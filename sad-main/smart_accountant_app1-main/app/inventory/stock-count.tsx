...
export default function StockCountScreen() {
  ...
  return (
    <View style={[styles.c, { paddingTop: insets.top }]}>
      <View style={styles.h}><TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>←</Text></TouchableOpacity><Text style={styles.t}>🔢 جرد المخزون</Text><View style={{width:40}}/></View>
      <FlatList data={products} keyExtractor={i=>i.id.toString()}
        renderItem={({item}) => (
          <View style={styles.row}><Text style={styles.name}>{item.name_ar}</Text><TextInput style={styles.qty} placeholder="الكمية الفعلية" placeholderTextColor="#666" keyboardType="numeric" textAlign="center" /></View>
        )}
