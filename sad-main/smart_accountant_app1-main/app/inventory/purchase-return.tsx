...
export default function SalesReturnScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    const last = await db.getFirstAsync("SELECT entry_number FROM journal_entries WHERE source_type='purchase_return' ORDER BY id DESC LIMIT 1") as any;
    let n = 1; if (last?.entry_number) n = parseInt(last.entry_number.split('-').pop()) + 1;
    setReturnNumber(`PRT-${String(n).padStart(5,'0')}`);
    setLoading(false);
  }, [db]);
  ...
  const saveReturn = async () => {
    ...
    setLoading(true);
    try {
      const customerAccount = await db.getFirstAsync("SELECT account_id FROM account_links WHERE module='suppliers' AND entity_id=?", [selectedCustomer.id]) as any;
      ...
      if (customerAccount && salesAccount) {
        await financialEngine.executeTransaction(db, {
          date: returnDate,
          description: description || `مرتجع مشتريات ${returnNumber}`,
          reference: returnNumber,
          source_type: 'purchase_return',
          currency_code: 'YER',
          exchange_rate: 1,
          lines: [
            { account_id: salesAccount.id, debit_original: totalAmount, credit_original: 0, description: 'مرتجع مشتريات' },
  ...
  return (
    <View style={[styles.c, { paddingTop: insets.top }]}>
      <View style={styles.h}><TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>←</Text></TouchableOpacity><Text style={styles.t}>↩️ مرتجع مشتريات</Text><Text style={styles.num}>{returnNumber}</Text></View>
